...
class Image:
    ...
    def medianDerterminant(self, image, currentImage):
        ...
        for pointx in range(0, 950):
            ...
            value_to_print = 0
            for x in range(1, len(midwhite) - 1):
                if midwhite[x] - midwhite[x + 1] <= self.minimum_gap_value and midwhite[x] - midwhite[x + 1] >= self.maximum_gap_value:   #-35 and -150 are the standard values
                    image[midwhite[x + 1]][pointx] = (0,255,0,-1) #green   bot green
                    image[midwhite[x]][pointx]     = (0,255,0,-1) #green   top green 
                    image[midwhite[0]][pointx]     = (255,0,0,-1) #blue    top blue
                    image[midwhite[-1]][pointx]    = (255,0,0,-1) #blue    bot blue

                    image_top.append(midwhite[0])
                    image_white_top.append(midwhite[x])
                    image_white_bot.append(midwhite[x + 1])
                    image_bot.append(midwhite[-1])
                    ...
                    if top_white_width >= self.min_gap_value and midwhite[0] != 0:
                        white_top.append(top_white_width)

                    #bottom white area of the retina
                    bot_white_width = midwhite[-1] - midwhite[x + 1]
                    if bot_white_width >= self.min_gap_value and midwhite[0] != 0:
                        white_bot.append(bot_white_width)

                    #inner area of the retina (between both white sections)
                    inner_point = midwhite[x + 1] - midwhite[x]
                    if inner_point >= self.min_gap_value and midwhite[0] != 0:
                        inner_distance.append(inner_point)

        # Testing settings
        # 21 30 2 105 -15 -215 1 1 155
        # 0 1 0 100 -35 -500 1 1 155
